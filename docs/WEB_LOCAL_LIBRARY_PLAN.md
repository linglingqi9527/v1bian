...
第一版资料包不要拆太多文件，先用一个 `bianleme-db.json` 保存用户元数据；音频、视频作为独立文件保存。
...
- `bianleme-db.json`：保存收藏、已看、赛评、训练元数据、设置等用户数据。
- `media/audio/`：保存录音文件。
- `media/video/`：保存录像文件。
- `backups/`：保存升级前备份、手动备份、自动备份。

第一版不建议把每篇赛评拆成单独文件。原因是当前应用还在快速调整，单文件数据库更容易迁移、校验和备份。后续如果赛评数量和训练数量明显变大，再考虑拆分。
...
[
  {
    ...
    "priority": "yellow",
    "mediaType": "audio",
    "mediaPath": "media/audio/training-001.webm",
    "durationMs": 120000,
    "createdAt": "2026-07-01T00:00:00.000Z",
    "updatedAt": "2026-07-01T00:00:00.000Z"
  }
...
- 相对路径：例如 `media/audio/training-001.webm`
- 时长：`durationMs`
- 文件大小：后续可选
- 创建时间和更新时间

第一版可以把媒体文件信息放在 `trainings` 里；如果后续一条训练支持多个素材，再升级为：
  ...
  "mediaItems": [
    {
      "id": "media-001",
      "mediaType": "audio",
      "mediaPath": "media/audio/media-001.webm",
      "durationMs": 120000
    }
  ]
}
...
- 初始化 `media/audio`、`media/video`、`backups`。
- 读取 `bianleme-db.json`。
- 写入 `bianleme-db.json`。
- 写入音频 / 视频文件。
...
阶段 8：录音 / 录像写入 `media/audio` 和 `media/video`。
...
```txt
未连接资料包：继续使用 localDb + IndexedDB 媒体缓存
已连接资料包：优先使用 bianleme-db.json + media/audio + media/video
```
